...
class Calendar
{
  ...
  private:
    ...
    static std::tm getNow();

    static bool isSameDay(const std::tm *date1, const std::tm *date2);
    static bool isSameDay(const std::tm *date1, int day, int month, int year);
    ...
    static bool handleToday(std::stringstream &ss, const tm &now, int day, int month, int year);
